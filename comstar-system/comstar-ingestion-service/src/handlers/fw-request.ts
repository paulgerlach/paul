...
class FirmwareRequestHandler {
  ...
  fileHandles: Map<any, any>;
  fileInfoCache: Map<any, any>;
  cacheTTL: number;

  downloadProgress: Map<any, any>;
  ...
  async ensureFirmwareDirectory() {
    try {
      ...
      await fs.mkdir(path.join(this.FIRMWARE_DIR, "upload"), {
        recursive: true,
      });
      await fs.mkdir(path.join(this.FIRMWARE_DIR, "active"), {
        recursive: true,
      });
      await fs.mkdir(path.join(this.FIRMWARE_DIR, "archive"), {
        recursive: true,
      });
    } catch (error) {
  ...
  async handle({ gatewayEui, data, messageNumber }) {
    ...
    try {
      ...
      if (chunkNumber < 0 || chunkNumber >= firmware.total_chunks) {
        throw new Error(
          `Invalid chunk number: ${chunkNumber} (max: ${firmware.total_chunks - 1})`,
        );
      }
      ...
      if (!this.isCompatible(firmware, gatewayEui)) {
        throw new Error(
          `Firmware ${safeFilename} not compatible with gateway ${gatewayEui}`,
        );
      }
      ...
      // Track download progress
      await this.trackDownloadProgress(
        gatewayEui,
        safeFilename,
        chunkNumber,
  ...
  async verifyAuthorization(gatewayEui, firmwareFile) {
    ...
    if (result.rows.length === 0) {
      ...
      const firmwareResult = await databaseService.query(firmwareQuery, [
        firmwareFile,
      ]);
  ...
  verifyChunkIntegrity(chunk, chunkNumber, firmware) {
    ...
    if (chunk.address >= maxAddress) {
      throw new Error(
        `Chunk address ${chunk.address} exceeds file size ${maxAddress}`,
      );
    }
  ...
  detectFirmwareType(filename) {
    if (filename.includes("boot") || filename.includes("mcuboot"))
      return "boot";
    if (filename.includes("modem") || filename.includes("mfw")) return "modem";
    if (filename.includes("app") || filename.includes("application"))
      return "application";
    return "unknown";
  }
  ...
  async uploadFirmware(fileBuffer, originalFilename, metadata = {}) {
    ...
    const version =
      metadata.version || this.extractVersionFromFilename(originalFilename);
  ...
  async getDownloadProgress(gatewayEui) {
    const progress = Array.from(this.downloadProgress.values()).filter(
      (p) => p.gatewayEui === gatewayEui,
    );
