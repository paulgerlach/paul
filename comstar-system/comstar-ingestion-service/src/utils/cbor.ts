...
class CborUtils {
  ...
  async decode(data) {
    try {
      ...
      try {
        ...
        throw new Error(
          `Decode failed: ${cborError.message}, ${jsonError.message}`,
        );
      }
    }
  }
  ...
  async extractMessage(topic, payload) {
    try {
      ...
      if (decoded.i.toLowerCase() !== gatewayEui.toLowerCase()) {
        console.warn(
          `⚠️  EUI mismatch: topic ${gatewayEui} != payload ${decoded.i}`,
        );
  ...
  async testRoundTrip(data) {
    try {
      ...
      console.log(
        "Encoded:",
        this.bufferToHex(encoded).substring(0, 100) + "...",
      );
      ...
      console.log(
        success ? "✅ Round-trip successful" : "❌ Round-trip failed",
      );
