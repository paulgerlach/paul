import type { DocCostCategoryType, LocalType } from "@/types";

export async function processInvoicesViaNext(
	files: File[],
	...
	fd.append("costCategories", JSON.stringify(costCategories));
	fd.append("apartments", JSON.stringify(locals));
...
export function mapCostCategoryToPurpose(
	...
	if (
		...
		return (
			purposeOptions.find((o) => o.toLowerCase().includes("reinigung")) ?? null
		);
	}
...
export function buildInvoiceNotes(invoice: any) {
	...
	if (invoice.building?.address)
		push("Gebäude Adresse", invoice.building.address);
	...
	if (
		Array.isArray(invoice.building_check?.indicators_found) &&
		invoice.building_check.indicators_found.length
	) {
		lines.push(
			`Indikatoren: ${invoice.building_check.indicators_found.join(", ")}`,
		);
	}
