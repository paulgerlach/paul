...
import AdminObjekteLocalItemDocResult from "@/components/Admin/ObjekteLocalItem/Admin/AdminObjekteLocalItemDocResult";
  ...
  return (
    <div className="py-6 px-9 h-[calc(100dvh-77px)] max-h-[calc(100dvh-77px)] max-xl:h-[calc(100dvh-53px)] max-xl:max-h-[calc(100dvh-53px)] grid grid-rows-[auto_1fr]">
      <Breadcrumb
        ...
        link={`${ROUTE_ADMIN}/${user_id}${ROUTE_BETRIEBSKOSTENABRECHNUNG}`}
      ...
      <ContentWrapper className="space-y-4">
        <div className="overflow-y-auto space-y-4">
          {locals?.map((local) => (
            <AdminObjekteLocalItemDocResult
              id={objekt_id}
              localID={local.id}
              userID={user_id}
              key={local.id}
