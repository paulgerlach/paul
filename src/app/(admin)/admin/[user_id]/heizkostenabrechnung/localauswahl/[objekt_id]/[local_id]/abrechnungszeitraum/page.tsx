import { getAdminContractsByLocalID, getLocalById } from "@/api";
...
export default async function AbrechnungszeitraumPage({
  ...
  params: Promise<{ objekt_id: string; local_id: string; user_id: string }>;
}) {
  const { objekt_id, local_id, user_id } = await params;
  ...
  return (
    <div className="py-6 px-9 h-[calc(100dvh-77px)] max-h-[calc(100dvh-77px)] max-xl:h-[calc(100dvh-53px)] max-xl:max-h-[calc(100dvh-53px)] grid grid-rows-[auto_1fr]">
      <Breadcrumb
        ...
        link={`${ROUTE_ADMIN}/${user_id}${ROUTE_HEIZKOSTENABRECHNUNG}/localauswahl/${objekt_id}`}
