...
export default async function GesamtkostenPage({
  ...
  params: Promise<{ objekt_id: string; doc_id: string; user_id: string, path_slug: string }>;
}) {
  const { objekt_id, doc_id, user_id, path_slug } = await params;
  ...
  const backLink = path_slug === "manuell" ? `${ROUTE_ADMIN}/${user_id}${ROUTE_HEIZKOSTENABRECHNUNG}/objektauswahl/${objekt_id}/abrechnungszeitraum` : `${ROUTE_ADMIN}/${user_id}${ROUTE_HEIZKOSTENABRECHNUNG}/objektauswahl/${objekt_id}/${doc_id}/${path_slug}/dokumentenmanagement`;
