...
export default function CSVUploadPage() {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `/api/upload-csv?filename=${encodeURIComponent(file.name)}`,
        {
          method: "POST",
          headers: {
    ...
    return (
      <div className="py-6 px-9 max-medium:px-4 max-medium:py-4 h-[calc(100dvh-77px)] flex items-center justify-center">
        <div className="text-center">
          <div className="text-lg max-medium:text-base text-gray-600">
  ...
  return (
    <div className="py-6 px-9 max-medium:px-4 max-medium:py-4 h-[calc(100dvh-77px)] max-h-[calc(100dvh-77px)] max-medium:h-auto max-medium:max-h-none overflow-y-auto">
      <Breadcrumb backTitle="Admin" link={ROUTE_ADMIN} title="CSV Upload" />

      <ContentWrapper className="space-y-6 max-medium:space-y-4">
        {/* Header */}
        <div>
          <h2 className="text-2xl max-medium:text-xl font-bold mb-2">
            Manual CSV Upload (Super Admin Only)
          </h2>
          <p className="text-gray-600 max-medium:text-sm">
            Upload meter reading CSV files from Engelmann Gateway. All data is
            permanently stored.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 max-medium:gap-3">
          <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 max-medium:p-3">
            <div className="text-sm max-medium:text-xs text-blue-600 font-medium">
              Total Records
            </div>
          ...
          <div className="bg-green-50 border border-green-200 rounded-lg p-4 max-medium:p-3">
            <div className="text-sm max-medium:text-xs text-green-600 font-medium">
              Parser Status
            </div>
            <div className="text-lg max-medium:text-base font-bold text-green-900">
              ✅ Fixed & Deployed
            </div>
          </div>
          <div className="bg-purple-50 border border-purple-200 rounded-lg p-4 max-medium:p-3">
            <div className="text-sm max-medium:text-xs text-purple-600 font-medium">
              Recent Uploads
            </div>
              ...
              <div className="p-4 max-medium:p-3 bg-white border border-gray-200 rounded-lg">
                <div className="flex items-center justify-between max-medium:flex-col max-medium:items-stretch max-medium:gap-3">
                  <div className="min-w-0">
                    <div className="font-medium text-gray-900 max-medium:text-sm truncate">
                      {file.name}
                    </div>
        ...
        {result && (
          <div
            className={`p-6 max-medium:p-4 rounded-lg border-2 ${
              result.error
            ...
            {result.error ? (
              ...
              <div className="space-y-4">
                ...
                <div className="bg-white border-2 border-gray-200 rounded-lg p-4 max-medium:p-3">
                  <div className="text-sm max-medium:text-xs font-semibold text-gray-700 mb-3">
                    📊 UPLOAD SUMMARY
                  </div>
                  ...
                  <div className="space-y-4 max-medium:space-y-3">
                    ...
                    <div className="bg-gradient-to-r from-blue-50 to-purple-50 rounded-lg p-4 max-medium:p-3">
                      <div className="flex items-baseline justify-between mb-2 max-medium:flex-col max-medium:items-start max-medium:gap-1">
                        <span className="text-gray-700 font-medium max-medium:text-sm">
                          📄 Total Rows in CSV File
                        </span>
                        <span className="text-2xl max-medium:text-xl font-bold text-gray-900">
                          {result.recordCount}
                        </span>
                      </div>
                    ...
                    <div className="space-y-3">
                      ...
                      <div className="grid grid-cols-2 gap-3 max-medium:gap-2">
                        <div className="bg-green-50 border border-green-200 rounded-lg p-3 max-medium:p-2">
                          ...
                          <div className="text-2xl max-medium:text-xl font-bold text-green-600">
                        ...
                        <div className="bg-purple-50 border border-purple-200 rounded-lg p-3 max-medium:p-2">
                          ...
                          <div className="text-2xl max-medium:text-xl font-bold text-purple-600">
                ...
                {result.insertedRecords === 0 &&
                  (result.skippedDuplicates || 0) > 0 && (
                    <div className="flex items-start gap-3 max-medium:gap-2 p-4 max-medium:p-3 bg-purple-50 border border-purple-200 rounded-lg">
                      <div className="text-2xl max-medium:text-xl flex-shrink-0">
                        💡
                      </div>
                      <div>
                        <div className="font-semibold text-purple-900 max-medium:text-sm">
                          This file was already uploaded
                        </div>
                        <div className="text-sm max-medium:text-xs text-purple-700">
                          All {result.skippedDuplicates || 0} data records were
                          skipped because they already exist in the database.
                          {(result.skippedHeaders || 0) > 0 &&
                            ` Additionally, ${result.skippedHeaders} header rows were filtered out.`}
                        </div>
                      </div>
                    </div>
                  )}
                ...
                {(result.insertedRecords || 0) > 0 && (
                  <div className="flex items-start gap-3 max-medium:gap-2 p-4 max-medium:p-3 bg-green-50 border border-green-200 rounded-lg">
                    ...
                    <div>
                      <div className="font-semibold text-green-900 max-medium:text-sm">
                        Successfully imported new data
                      </div>
                      <div className="text-sm max-medium:text-xs text-green-700">
                        {result.insertedRecords || 0} new records were added to
                        the database.
                        {(result.skippedDuplicates || 0) > 0 &&
                          ` ${result.skippedDuplicates} duplicates were skipped.`}
                      </div>
                    </div>
                  </div>
                )}
        ...
        <div className="bg-white border border-gray-200 rounded-lg p-6 max-medium:p-4">
          <h2 className="text-xl max-medium:text-lg font-bold mb-4 max-medium:mb-3">
            Recent Uploads History ({totalRecords.toLocaleString()} total
            records)
          </h2>

          {loadingHistory ? (
            <div className="text-center py-8 max-medium:py-4 text-gray-500 max-medium:text-sm">
              Loading history...
            </div>
          ) : (
            <div className="space-y-6 max-medium:space-y-4">
              {Object.entries(uploadsByDate).map(([date, uploads]) => (
                <div key={date}>
                  <h3 className="text-lg max-medium:text-base font-semibold text-gray-700 mb-2">
                    {date}
                  </h3>
