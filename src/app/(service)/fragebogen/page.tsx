...
export default function FragebogenPage() {
	...
	const [isUnder50Flow, setIsUnder50Flow] = useState(false);
	const [isSubmitStep, setIsSubmitStep] = useState(false);
	const [lastStep, setLastStep] = useState<number | null>(null);

	// Under50 flow submits on step 5, Over50 flow submits on step 6
	// Under50 = 5 steps, Over50 = 6 steps

	useEffect(() => {
		setIsUnder50Flow(propertyCategory === "1-50 Immobilien");
	}, [propertyCategory]);

	useEffect(() => {
		setIsSubmitStep(isUnder50Flow ? activeStep === 5 : activeStep === 6);
	}, [isUnder50Flow, activeStep]);

	useEffect(() => {
		const _lastStep = isUnder50Flow ? 5 : 6;
		setLastStep(_lastStep);
		setTotalSteps(_lastStep + 1);
	}, [isUnder50Flow, setTotalSteps]);
		...
		<main id="content" className="pt-8 pb-24 px-5">
			{isSubmited ? (
				...
				<Fragment>
					...
					>
						{new Array(totalSteps).fill(true).map((_, step) => (
							<span
								key={step}
					...
					<div className="questionare-steps max-w-6xl mx-auto flex items-start max-large:flex-col max-large:gap-10 max-small:gap-4 justify-between">
						...
						>
							...
							{step0Error && (activeStep === 0 || activeStep === 1) && (
							...
							<div className="flex items-center justify-start gap-5">
								{(() => {
									const handleClick = () => {
										// Validate step 0 & 1 - customer type and property count  is required

										if (
											(activeStep === 0 && !customerType) ||
											(activeStep === 1 && !propertyCategory)
										) {
											setStep0Error(true);
											return;
										}
									...
									return (
										...
										>
											{isSubmitStep && mutation.isPending
												? "Wird gesendet..."
												: "Bestätigen"}
										</button>
									);
