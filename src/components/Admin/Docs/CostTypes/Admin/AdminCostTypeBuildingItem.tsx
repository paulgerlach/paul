...
export default function AdminCostTypeBuildingItem({
  isOpen,
  index,
  onClick,
  type,
  ...
  const handleOpenDialog = () => {
    ...
    setPurposeOptions();
    openDialog(`admin_${type.type}_betriebskostenabrechnung_upload`);
  };
    ...
    <div className={`bg-[#F5F5F5] rounded-md ${isOpen ? `active` : ""}`}>
      ...
      >
        ...
        >
          ...
          <p className="font-semibold max-xl:text-sm max-medium:text-xs text-dark_green truncate">{type.name}</p>
        </div>
        <div className="flex items-center whitespace-nowrap justify-end gap-7 max-medium:gap-2 flex-shrink-0 ml-2">
          ...
          <TheeDotsCostTypeButton
            editDialogAction="admin_cost_type_betriebskostenabrechnung_edit"
            itemID={type.id ?? ""}
            userID={type.user_id}
            deleteDialogAction="admin_cost_type_betriebskostenabrechnung_delete"
          />
        </div>
      </div>
      <div
