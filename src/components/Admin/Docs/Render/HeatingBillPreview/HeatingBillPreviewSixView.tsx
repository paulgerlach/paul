...
export interface HeatingBillPreviewSixCalculated {
    energySource: string;
    energyConsumptionTotal: number; // kWh
    co2EmissionFactor: number; // kg CO2/kWh
    co2EmissionTotal: number; // kg

    unitHeatingConsumption: number; // kWh
    unitWaterConsumption: number; // kWh
    unitTotalConsumption: number; // kWh
    unitLivingSpace: number; // m²
    unitConsumptionPerSqm: number; // kWh/m²

    avgConsumptionPerSqm: number; // kWh/m² (Bundesweiter Vergleich)
    buildingAvgConsumptionPerSqm: number; // kWh/m² (Liegenschaftsdurchschnitt)

    heatingData: any[];
    waterData: any[];
}

export const HeatingBillPreviewSixView = ({
    previewData,
    data,
}: {
    previewData: HeatingBillPreviewData;
    data: HeatingBillPreviewSixCalculated;
}) => {
    ...
    const exactFormatter = new Intl.NumberFormat("de-DE", {
        ...
        <div className="mx-auto max-w-[1400px] space-y-6 font-sans text-sm">
                    ...
                    <tbody>
                        <tr>
                            ...
                            <td className="py-1 px-2 font-bold text-pdf-dark rounded-r-base border border-l-0 border-pdf-dark text-right">
                                {formatter.format(data.energyConsumptionTotal)} kWh
                            </td>
                        </tr>
                        <tr>
                            ...
                            <td className="py-1 text-right">{exactFormatter.format(data.co2EmissionFactor)} kg CO2/kWh</td>
                            <td />
                        </tr>
                        ...
                        <tr>
                            ...
                            <td className="py-1 text-right px-2 rounded-r-base border border-l-0 border-pdf-dark text-pdf-dark font-bold">
                                {formatter.format(data.co2EmissionTotal)} kg
                            </td>
                        </tr>
                    </tbody>
                </table>
            ...
            <div className="grid grid-cols-2 gap-8 text-xs">
                <div>
                    ...
                    <BarChart width={300} height={200} data={data.heatingData}>
                ...
                <div>
                    ...
                    <BarChart width={300} height={200} data={data.waterData}>
                        <CartesianGrid strokeDasharray="3 3" />
                        <XAxis dataKey="name" />
                        <YAxis />
                        <Tooltip />
            ...
            <div className="text-sm grid grid-cols-2 gap-10 mt-4">
                <table className="w-full text-pdf-text border-spacing-0 border-separate mb-2">
                    <tbody>
                        <tr>
                            ...
                            <td />
                            <td className="py-1 text-right">{formatter.format(data.unitHeatingConsumption)} kWh</td>
                        </tr>
                        <tr>
                            ...
                            <td className="py-1 border-b border-pdf-dark pb-2 text-right">
                                {formatter.format(data.unitWaterConsumption)} kWh
                            </td>
                        </tr>
                        <tr>
                            <td className="py-1">GESAMT</td>
                            <td />
                            <td className="py-1 text-right">{formatter.format(data.unitTotalConsumption)} kWh</td>
                        </tr>
                        <tr>
                            <td className="py-1 pb-2">Ihre Wohnfläche</td>
                            <td className="pb-2" />
                            <td className="py-1 pb-2 text-right">{formatter.format(data.unitLivingSpace)} m²</td>
                        </tr>
                        <tr className="text-pdf-dark font-bold">
                            ...
                            <td className="py-1 border-pdf-dark border border-l-0 rounded-r-base px-2 text-right">
                                {formatter.format(data.unitConsumptionPerSqm)} kWh / m²
                            </td>
                        </tr>
                    </tbody>
                </table>
                <table className="w-full text-pdf-text border-spacing-0 border-separate mb-2 ">
                    <tbody>
                        ...
                        <tr className="text-pdf-dark font-bold">
                            <td className="py-1 border-pdf-dark border border-r-0 rounded-l-base px-2">
                                {formatter.format(data.avgConsumptionPerSqm)} kWh/ m²
                            </td>
                            <td className="py-1 border-pdf-dark border border-l-0 rounded-r-base px-2 text-right">
                                {formatter.format(data.buildingAvgConsumptionPerSqm)} kWh/m²
                            </td>
                        </tr>
                    </tbody>
                </table>
