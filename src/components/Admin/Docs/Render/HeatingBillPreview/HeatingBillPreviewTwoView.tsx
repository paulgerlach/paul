                                ...
                                {costGroups.energy.map((inv, idx) => (
                                    <tr key={inv.id || idx}>
                                        <td className="py-1 px-2">
                                            ...
                                            {inv.notes && <div className="text-[10px] text-pdf-text italic">{inv.notes}</div>}
                                        </td>
                        ...
                        <table className="w-full">
                            ...
                            <tbody>
                                <tr>
                                    ...
                                    <td></td>
                                    <td className="py-1 px-2 text-right">{formatEuro(sums.energySum)}</td>
                                </tr>
                ...
                <table className="w-full">
                    ...
                    <tbody>
                        {costGroups.separate.map((inv, idx) => {
                            const category = previewData.costCategories.find(c => c.type === inv.cost_type);
                            return (
                                <tr key={`sep-${idx}`}>
                                    <td className="py-1 px-2 font-medium">{category?.allocation_key || "Warmwasser/Kaltwasser"}</td>
                                    <td className="py-1 px-2">{inv.cost_type}</td>
                                    <td className="py-1 px-2 text-center text-xs">
                                        {inv.invoice_date ? formatDateGerman(inv.invoice_date) : ""}
                                    </td>
                ...
                <div className="">
                    ...
                    <div className="flex items-center justify-between mt-2">
                        <div className="flex flex-col items-center">
                            <span>2,5 kWh/m³/K x {formatter.format(thermal.totalWaterHotm3)} m³ x (60-10°C)</span>
                            <div className="text-center w-full font-bold border-t border-white mt-2 pt-2">
                                1,15
                            </div>
                        </div>
                        <span>= {formatter.format(thermal.warmWaterEnergykWh)} kWh Nah-/Fernwärme</span>
                        <span>= {formatter.format(thermal.warmWaterPercent)}% d. Gesamtverbr.</span>
                    </div>
                </div>
            </div>
            ...
            <div className="mt-6 space-y-8">
                <div>
                    <div className="flex justify-between text-pdf-text">
                        <div>
                            {formatter.format(thermal.warmWaterPercent)}% aus {formatEuro(sums.energyAndHeatingSum)} Energie- und Heizungsbetriebskosten
