...
import { editAdminObjekt } from "@/actions/edit/editAdminObjekt";
...
export type AdminEditObjekteFormValues = z.infer<typeof objectSchema>;

type EditObjekteFormProps = {
  objekteID: string;
  initialValues?: AdminEditObjekteFormValues;
};

export default function AdminEditObjekteForm({
  objekteID,
  initialValues,
}: EditObjekteFormProps) {
  const router = useRouter();
  const methods = useForm<AdminEditObjekteFormValues>({
    ...
    <Form {...methods}>
      <form
        ...
        onSubmit={methods.handleSubmit(async (data) => {
          try {
            await editAdminObjekt(objekteID, data);
            toast.success("Erfolgreich aktualisiert");
            router.push(`${ROUTE_ADMIN}${ROUTE_OBJEKTE}`);
            methods.reset();
          } catch (err) {
      ...
      >
        ...
        <div className="w-full border-b py-5 space-y-5 border-dark_green/10">
          ...
          <FormSelectField<AdminEditObjekteFormValues>
            control={methods.control}
        ...
        <FormImageUpload<AdminEditObjekteFormValues>
          control={methods.control}
          name="image_url"
          objektId={objekteID}
        />
        ...
        <FormTechnicalEquipment<AdminEditObjekteFormValues>
          control={methods.control}
        />
        <Button
