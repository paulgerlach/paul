...
export default function ObjekteItemLocalDocWithHistory({
  item,
  isOpen,
  index,
  onClick,
  ...
  return (
    ...
    >
      <ObjekteItemLocalHeader
        ...
        link={`${ROUTE_HEIZKOSTENABRECHNUNG}/localauswahl/${objektID}/${item.id}/abrechnungszeitraum`}
        isOpen={isOpen}
      />
      <div
        ref={contentRef}
        ...
        {relatedOpenedDocuments?.map((doc) => (
          <div className="flex items-center justify-between" key={doc.id}>
            <Link
              ...
              href={`${ROUTE_HEIZKOSTENABRECHNUNG}/localauswahl/weitermachen/${doc.id}/abrechnungszeitraum`}
            >
              <Image
                width={0}
                height={0}
