...
export default function ObjekteLocalItem({
  ...
  const { status, renderStatusImage, renderStatusBadge } = useLocalStatus(contracts);

  return (
    <div
      className={`bg-white/50 rounded-2xl max-medium:rounded-xl ${isOpen ? `active` : ""} ${status === "vacancy" && "available"} [.available.active]:pb-7`}>
      <div
        ...
        <div className="flex items-center justify-start gap-8 max-medium:gap-2 flex-1 min-w-0">
          ...
          <div
            ...
            <p className="text-2xl max-xl:text-lg max-medium:text-xs text-dark_green max-medium:leading-tight">{buildLocalName(item)}</p>
            <Image
              width={0}
              height={0}
              sizes="100vw"
