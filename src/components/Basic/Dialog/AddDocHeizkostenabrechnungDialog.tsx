...
export default function AddDocHeizkostenabrechnungDialog() {
  ...
  const isOpen = Object.entries(openDialogByType).some(
    ([key, value]) => key.endsWith("_upload") && value === true
  );
      ...
      <Form {...methods}>
        <form
          ...
          <div className="gap-2 grid grid-cols-2">
            <p className="text-lg text-admin_dark_text">
            ...
            <div className="rounded-full h-fit w-fit p-0.5 bg-[#EAEAEA] flex items-center justify-center mr-0 ml-auto">
              <button
                ...
                className={`text-admin_dark_text text-lg py-1 px-8 rounded-full ${servicePeriod === false ? "bg-white" : "bg-[#EAEAEA]"} cursor-pointer transition-all duration-300`}>
                Nein
              </button>
              <button
                type="button"
                onClick={() => methods.setValue("service_period", true)}
                className={`text-admin_dark_text text-lg py-1 px-8 rounded-full ${servicePeriod === true ? "bg-white" : "bg-[#EAEAEA]"} cursor-pointer transition-all duration-300`}>
                Ja
              </button>
            </div>
          </div>
          <div className="space-y-1.5">
            ...
            <div className="px-3.5 py-4 border border-black/20 rounded-md">
