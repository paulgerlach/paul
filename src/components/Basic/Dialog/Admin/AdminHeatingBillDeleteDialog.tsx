...
import { adminDeleteHeatingBillDocument } from "@/actions/delete/admin/adminDeleteHeatingBillDocument";

export default function AdminHeatingBillDeleteDialog() {
  const {
    itemID,
    openDialogByType,
    closeDialog,
  ...
  const isOpen = openDialogByType.admin_heating_bill_delete;
  ...
  const deleteMutation = useMutation({
    mutationFn: async (id: string) => {
      return await adminDeleteHeatingBillDocument(id, String(user_id));
    },
    onSuccess: (data) => {
      if (data.success) {
        ...
        closeDialog("admin_heating_bill_delete");
  ...
  if (isOpen && !!itemID)
    return (
      <DialogBase dialogName="admin_heating_bill_delete">
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <button
            ...
            onClick={() => closeDialog("admin_heating_bill_delete")}
          >
            Abbrechen
          </button>
        </div>
