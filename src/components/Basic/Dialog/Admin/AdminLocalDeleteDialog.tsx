...
export default function AdminLocalDeleteDialog() {
  const { itemID, openDialogByType, closeDialog } = useDialogStore();
  const isOpen = openDialogByType.admin_local_delete;
  ...
  const handleDelete = async () => {
    if (!!itemID) {
      ...
      if (res.success) {
        closeDialog("admin_local_delete");
        router.refresh();
      }
    }
  };
  ...
  if (isOpen && !!itemID)
    return (
      <DialogBase dialogName="admin_local_delete">
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <button
            ...
            onClick={() => closeDialog("admin_local_delete")}
          >
            Abbrechen
          </button>
        </div>
