...
export default function BlogFilters() {
  ...
  return (
    <div className="flex flex-col gap-8 max-w-7xl mx-auto px-4 min-h-dvh py-8">
      <div className="flex items-center justify-start gap-4 mb-4 flex-wrap">
        {cachedTags?.map((tag) => (
          <button
            key={tag}
            className={`cursor-pointer flex items-center justify-center gap-1.5 text-center w-fit py-2 max-medium:py-3 px-5 text-sm text-dark_text rounded-halfbase transition-all duration-300 hover:bg-green/80 active:bg-green/90 disabled:bg-green/50 disabled:cursor-not-allowed ${
