...
export default function ContactForm() {
  ...
  const mutation = useMutation({
    ...
    onSuccess: () => {
      setFormMessage({ text: "Ihre Nachricht wurde erfolgreich gesendet!", type: "success" });
      reset();
    },
    onError: () => {
      setFormMessage({
  ...
  return (
    <div className="pt-8 max-small:pt-8 px-[100px] max-large:px-20 max-medium:px-10 max-small:px-5">
      ...
      <form onSubmit={handleSubmit((data) => mutation.mutate(data))}
        ...
        {errors.infoChecked && <p className="text-red-500 text-sm">{errors.infoChecked.message}</p>}
