...
import { objekte, operating_cost_documents, locals, contracts, contractors, invoice_documents, users_in_auth, users } from "./schema";
...
export const invoice_documentsRelations = relations(invoice_documents, ({one}) => ({
	...
	local: one(locals, {
		fields: [invoice_documents.direct_local_id],
		references: [locals.id]
	}),
