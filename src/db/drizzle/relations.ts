...
import { locals, heating_bill_documents, objekte, operating_cost_documents, contracts, contractors, local_meters, invoice_documents, users, heating_invoices } from "./schema";

export const heating_bill_documentsRelations = relations(heating_bill_documents, ({ one, many }) => ({
	local: one(locals, {
	...
	heating_invoices: many(heating_invoices),
}));
...
export const operating_cost_documentsRelations = relations(operating_cost_documents, ({ one, many }) => ({
	objekte: one(objekte, {
...
export const invoice_documentsRelations = relations(invoice_documents, ({ one }) => ({
	objekte: one(objekte, {
...
export const heating_invoicesRelations = relations(heating_invoices, ({ one }) => ({
	heating_bill_document: one(heating_bill_documents, {
		fields: [heating_invoices.heating_doc_id],
		references: [heating_bill_documents.id]
	}),
	objekte: one(objekte, {
		fields: [heating_invoices.objekt_id],
		references: [objekte.id]
	}),
}));