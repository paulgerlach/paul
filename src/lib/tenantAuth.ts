...
const SESSION_SECRET =
	process.env.TENANT_SESSION_SECRET || process.env.SUPABASE_SERVICE_ROLE_KEY!;
...
export async function getTenantMeterIds(tenantId: string): Promise<string[]> {
	// this function has been deleted from pg????
	// const { data, error } = await supabaseAdmin.rpc('get_tenant_meter_ids', {
	//   tenant_id: tenantId,
	// });

	// console.log("axxxx => ", data, error)

	// WHAT THE FUCK IS THIS CODE??????
	// THIS IS A FUCKING JOIN IN TS? WHY NOT JUST WRITE PROPER SQL -- HOSS
	// if (error) {
	...
	if (!local?.meter_ids) return [];

	// console.log(local.meter_ids)
	const { data, error } = await supabaseAdmin
		.from("parsed_data")
		.select("local_meter_id")
		.in("device_id", local.meter_ids);
  ...
  // this is a disgusting hacky way of doing this, but due to time constraints
  // I had to do it this way.
	const rows = data as { local_meter_id: string }[];
	const unique = [...new Set(rows.map((r) => r.local_meter_id))];

	console.log("meters => ", data);

	return unique;
	// return parsed_data?.local_meter_id || [];
